...
class FrameRecord:
    ...
    num_violations: int
    violation_types: str  # comma-separated
...
class ScenarioResult:
    ...
    true_positives: int = 0       # correctly triggered on real hazard
    false_positives: int = 0      # triggered when no real hazard
    true_negatives: int = 0       # correctly did NOT trigger (safe frame)
    false_negatives: int = 0      # missed a real hazard

    # Timing
    response_time_ms: float = 0.0       # violation → brake trigger latency
    mean_monitor_latency_ms: float = 0.0

    # Safety score
    min_safety_score: float = 1.0
    mean_safety_score: float = 1.0
...
class MetricsLogger:
    ...
    def __init__(self, output_dir: str, scenario_name: str, monitor_type: str):
        ...
        self._result = ScenarioResult(
            scenario_name=scenario_name, monitor_type=monitor_type
        )

        # CSV writer
        csv_path = self.output_dir / f"{scenario_name}_{monitor_type}_frames.csv"
        self._csv_file = open(csv_path, "w", newline="")
        fieldnames = list(FrameRecord.__dataclass_fields__.keys())
        self._csv_writer = csv.DictWriter(self._csv_file, fieldnames=fieldnames)
        self._csv_writer.writeheader()

    def log_frame(self, record: FrameRecord) -> None:
        """Append a frame record."""
        self._frame_records.append(record)
        self._csv_writer.writerow(asdict(record))
        self._csv_file.flush()

    def mark_ground_truth_hazard(self, frame_id: int, is_hazard: bool) -> None:
        ...
        """
        if frame_id >= len(self._frame_records):
            return
        rec = self._frame_records[frame_id] if frame_id < len(self._frame_records) else None
        if rec is None:
            return
    ...
    def set_response_time(self, response_ms: float) -> None:
        self._result.response_time_ms = response_ms

    def set_stopping_info(self, distance_m: float, speed_mps: float) -> None:
        self._result.stopping_distance_m = distance_m
        self._result.vehicle_speed_at_trigger_mps = speed_mps

    def finalise(self) -> ScenarioResult:
        ...
        summary = {
            ...
            "mean_monitor_latency_ms": round(r.mean_monitor_latency_ms, 2),
            "min_safety_score": round(r.min_safety_score, 4),
            "mean_safety_score": round(r.mean_safety_score, 4),
            "stopping_distance_m": round(r.stopping_distance_m, 2),
            "vehicle_speed_at_trigger_mps": round(r.vehicle_speed_at_trigger_mps, 2),
        }
        with open(summary_path, "w") as f:
            json.dump(summary, f, indent=2)

        logger.info("Scenario '%s' [%s] — TPR=%.2f  FPR=%.2f  F1=%.2f  response=%.1fms",
...
def depth_to_colormap(depth: np.ndarray, max_depth: float = 50.0) -> np.ndarray:
...
def residual_to_colormap(
    residual: np.ndarray, max_val: float = 5.0
) -> np.ndarray:
